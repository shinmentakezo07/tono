...
export type NvidiaEditOutletContext = {
...
const buildBaseline = (form: OpenAIFormState, testModel: string): OpenAIEditBaseline => ({
...
export function AiProvidersNvidiaEditLayout() {
    ...
    if (initialData) {
      ...
      const initialTestModel =
        initialData.testModel && available.includes(initialData.testModel)
          ...
          : available[0] || '';
      const baseline = buildBaseline(seededForm, initialTestModel);
      initDraft(draftKey, {
        baseline,
        form: seededForm,
      ...
      initDraft(draftKey, {
        baseline: buildBaseline(emptyForm, ''),
        form: emptyForm,
        testModel: '',
        testStatus: 'idle',
        testMessage: '',
  ...
  const mergeDiscoveredModels = useCallback(
    (selectedModels: ModelInfo[]) => {
      ...
      if (addedCount > 0) {
        showNotification(t('ai_providers.nvidia_models_fetch_added', { count: addedCount }), 'success');
      }
    },
  ...
  const editorRootPath = useMemo(() => {
    if (hasIndexParam) {
      return `/ai-providers/nvidia/${params.index ?? ''}`;
    }
    return '/ai-providers/nvidia/new';
  ...
  const handleSave = useCallback(async () => {
    ...
    if (!name || !baseUrl) {
      showNotification(t('notification.nvidia_provider_required'), 'error');
      return;
    }
    ...
    try {
      ...
      showNotification(
        editIndex !== null
          ? t('notification.nvidia_provider_updated')
          : t('notification.nvidia_provider_added'),
        'success'
      );
      allowNextNavigation();
      setDraftBaseline(draftKey, buildBaseline(form, testModel));
      handleBack();
    } catch (err: unknown) {
  ...
  return (
    <Outlet
      ...
      } satisfies NvidiaEditOutletContext}
    />
  );
}
