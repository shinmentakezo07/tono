...
const NVIDIA_TEST_TIMEOUT_MS = 30_000;
...
export function AiProvidersNvidiaEditPage() {
  ...
  const title = hasIndexParam
    ? t('ai_providers.nvidia_edit_modal_title')
    : t('ai_providers.nvidia_add_modal_title');
  ...
  const runSingleKeyTest = useCallback(
    async (keyIndex: number): Promise<boolean> => {
      ...
      if (!baseUrl) {
        showNotification(t('ai_providers.nvidia_test_url_required'), 'error');
        return false;
      }
      ...
      if (!keyEntry?.apiKey?.trim()) {
        setDraftKeyTestStatus(keyIndex, { status: 'error', message: t('ai_providers.nvidia_test_key_required') });
        return false;
      }

      const modelName = testModel.trim() || availableModels[0] || '';
      if (!modelName) {
        showNotification(t('ai_providers.nvidia_test_model_required'), 'error');
        return false;
      }
      ...
      try {
        const result = await apiCallApi.request(
          ...
          { timeout: NVIDIA_TEST_TIMEOUT_MS }
        );
        ...
        const errorMessage = isTimeout
          ? t('ai_providers.openai_test_timeout', { seconds: NVIDIA_TEST_TIMEOUT_MS / 1000 })
          : message;
  ...
  const testAllKeys = useCallback(async () => {
    ...
    const baseUrl = form.baseUrl.trim();
    if (!baseUrl) {
      const message = t('ai_providers.nvidia_test_url_required');
      setTestStatus('error');
    ...
    const modelName = testModel.trim() || availableModels[0] || '';
    if (!modelName) {
      const message = t('ai_providers.nvidia_test_model_required');
      setTestStatus('error');
      setTestMessage(message);
      showNotification(message, 'error');
      return;
    ...
    if (validKeyIndexes.length === 0) {
      const message = t('ai_providers.nvidia_test_key_required');
      setTestStatus('error');
      setTestMessage(message);
      showNotification(message, 'error');
      return;
              ...
              <div key={index} className={styles.keyTableRow}>
                ...
                <div className={styles.keyTableColKey}>
                  <input
                    ...
                    placeholder={t('ai_providers.nvidia_key_placeholder')}
                  />
                </div>
      ...
      <Card>
        {invalidIndexParam || invalidIndex ? (
          ...
          <div className={styles.openaiEditForm}>
            <div className={styles.sectionHint} style={{ marginBottom: '12px', padding: '8px 12px', background: 'var(--bg-secondary, #f0fdf4)', border: '1px solid var(--border-subtle, #bbf7d0)', borderRadius: '6px', color: 'var(--text-secondary, #166534)' }}>
              {t('ai_providers.nvidia_models_hint')}
            </div>
            <Input
              label={t('ai_providers.nvidia_add_modal_name_label')}
              value={form.name}
              onChange={(e) => setForm((prev) => ({ ...prev, name: e.target.value }))}
              disabled={hasIndexParam || saving || disableControls || isTestingKeys}
              placeholder={t('ai_providers.nvidia_add_modal_name_placeholder')}
            />
            <Input
            ...
            />
            <Input
              label={t('ai_providers.nvidia_add_modal_url_label')}
              value={form.baseUrl}
              onChange={(e) => setForm((prev) => ({ ...prev, baseUrl: e.target.value }))}
              disabled={saving || disableControls || isTestingKeys}
              placeholder={t('ai_providers.nvidia_add_modal_url_placeholder')}
            />
            ...
            <div className={styles.modelConfigSection}>
              <div className={styles.modelConfigHeader}>
                <label className={styles.modelConfigTitle}>
                  {hasIndexParam
                    ? t('ai_providers.nvidia_edit_modal_models_label')
                    : t('ai_providers.nvidia_add_modal_models_label')}
                </label>
                <div className={styles.modelConfigToolbar}>
                  ...
                  <Button
                    variant="secondary"
                    size="sm"
                    onClick={openModelDiscovery}
                    disabled={saving || disableControls || isTestingKeys}
                  >
                    {t('ai_providers.nvidia_models_fetch_button')}
                  </Button>
                </div>
              </div>

              <div className={styles.sectionHint}>{t('ai_providers.nvidia_models_hint')}</div>

              <ModelInputList
                entries={form.modelEntries}
                onChange={(entries) => setForm((prev) => ({ ...prev, modelEntries: entries }))}
                namePlaceholder={t('ai_providers.nvidia_model_name_placeholder')}
              ...
              <div className={styles.modelTestPanel}>
                <div className={styles.modelTestMeta}>
                  <label className={styles.modelTestLabel}>{t('ai_providers.nvidia_test_title')}</label>
                  <span className={styles.modelTestHint}>{t('ai_providers.nvidia_test_hint')}</span>
                </div>
                <div className={styles.modelTestControls}>
                  <Select
                    ...
                    ariaLabel={t('ai_providers.nvidia_test_title')}
            ...
            <div className={styles.keyEntriesSection}>
              <div className={styles.keyEntriesHeader}>
                <label className={styles.keyEntriesTitle}>{t('ai_providers.nvidia_add_modal_keys_label')}</label>
                <span className={styles.keyEntriesHint}>{t('ai_providers.nvidia_keys_hint')}</span>
              </div>
