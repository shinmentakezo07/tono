...
export function NvidiaSection({
  ...
  const sortedConfigs = useMemo<IndexedProvider[]>(() => {
  ...
  const renderToolbar = (isFloating = false) => {
    ...
    return (
      <div className={styles.cardHeaderActions}>
        ...
        >
          {t('ai_providers.nvidia_add_button')}
        </Button>
      </div>
    );
  };
  ...
  const renderStaticTitle = () => (
    <span className={styles.cardTitle}>
      ...
      />
      {t('ai_providers.nvidia_title')}
    </span>
  );

  const renderProviderCard = ({ config: provider, originalIndex }: IndexedProvider) => {
    ...
    return (
      <div
        key={`nvidia-provider-${originalIndex}`}
      ...
      >
        <div className={styles.openaiProviderMeta}>
          ...
          {provider.testModel && (
            <div className={styles.fieldRow}>
              <span className={styles.fieldLabel}>{t('ai_providers.nvidia_test_model')}:</span>
      ...
      <div ref={sectionRef}>
        ...
        >
          {loading && sortedConfigs.length === 0 ? (
            ...
            <EmptyState
              title={t('ai_providers.nvidia_filtered_empty_title')}
              description={t('ai_providers.nvidia_filtered_empty_desc')}
              action={
                <Button
                  variant="secondary"
                  size="sm"
            ...
            <EmptyState
              title={t('ai_providers.nvidia_empty_title')}
              description={t('ai_providers.nvidia_empty_desc')}
            />
          ) : (
