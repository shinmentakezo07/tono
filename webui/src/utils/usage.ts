...
export function filterUsageByTimeRange<T>(
  ...
  if (range === 'all') {
    // Compute aggregates for the unfiltered data so top-level fields are always present.
    ...
    Object.values(apis).forEach((apiEntry) => {
      ...
      Object.values(models).forEach((modelEntry) => {
        ...
        details.forEach((detail) => {
          ...
          totalSummary.totalRequests += 1;
          if (detailRecord.failed === true) {
            totalSummary.failureCount += 1;
          } else {
            totalSummary.successCount += 1;
          }
          totalSummary.totalTokens += extractTotalTokens(detailRecord);
        });
      });
    });
    return {
